 ...
 *)
    (* Ans: 
        Except [d (b a 5)] & [e bool (b c 0)], others are well-typed
        elements of [grumble X] for some type [X]*)
...
match l with
  |[] => ([], [])
  | (x1, y1)::t => (x1::fst(split t), y1::snd(split t))
end.
...
Lemma f_comm : forall (X Y:Type) (f:X->Y) (l: list X) (n: X),
...
Proof.
  ...
  induction l.
  - simpl. reflexivity.
  - simpl. rewrite f_comm. rewrite <- IHl. reflexivity.
...
(* Ans:
    There are some functions that inputs and outputs are different types, such as [remove_one] and [remove_all]. They take type nat as [X], but a different type bag as [Y]. So it would be useful for [X] and [Y] to be different to apply [fold] on more kinds of situations. *)
...
Lemma prod_expand : forall (X Y :Type)(p:X*Y),
  (fst p, snd p) = p.
Proof.
  intros.
  simpl.
...
(* 
  Ans:
   Proof: 
    - First, suppose [ l = []] . 
        Then we get length l = 0

        It's easy to know that @nth_erro X [] 0 = None

        Because there is nothing in the list l.

    - Next, suppose [l = x::l'], where

        we suppose that when lenth l'=n s.t. @nth_error X l' n = None.

        length l = length (x::l') = length l' +1 = n + 1 
       
        @nth_error X l (n+1) means that we want to get the index = n+1 
        (n+2 th) X of list l. However, length l = n+1. Hence, it's   
        impossible to get the n+2 th X of list l.
        
        @nth_error X l n+1 = None.
        

      By the induction hypothesis, we know that  forall X n l, 
