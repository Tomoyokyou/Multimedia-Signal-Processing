...
Fixpoint nonzeros (l:natlist) : natlist :=
  match l with  
  | nil => nil
  | h::t => if beq_nat 0 h then nonzeros t else h :: nonzeros t
end.
...
Fixpoint count (v:nat) (s:bag) : nat :=
  match s with 
  |[] => 0
  |n::s' => if beq_nat n v then S(count v s') else count v s'
  end.
...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  (* When remove_one is applied to a bag without the number to remove,
    ...
    match s with
    |[]=>[]
    |n::s' => if beq_nat n v then s' else n::(remove_one v s')
    end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | [] => []
  | n::s' => if beq_nat v n then remove_all v s' else n::(remove_all v s')
end.
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
  | [] => true
  | n::s1' => if beq_nat (count n s2) 0 then false else subset s1' (remove_one n s2)
end.
...
Proof.
  ...
  - rewrite <- H. simpl. rewrite <- beq_nat_refl. reflexivity.
Qed.
(** [] *)
...
Lemma rev_id : forall l : natlist, forall n : nat,
  rev (l++[n]) = n::(rev l).
Proof.
   intros.
   induction l.
...
Proof.
  ...
  induction l.
  -reflexivity.
  -simpl. rewrite -> rev_id. rewrite -> IHl. reflexivity.
Qed.
...
Lemma removeOneZero : forall s : bag,
      remove_one 0 (0::s) = s.
Proof.
      intros s.
      induction s.
...
Proof.
  ...
  - simpl. reflexivity.
  - induction n
    + simpl. rewrite -> removeOneZero. simpl. rewrite ble_n_Sn with (n:= count 0 s). reflexivity.
...
Theorem rev_surjective : forall (l1 l2 : natlist), 
        l1 = l2 -> rev l1 = rev l2.
Proof.
    intros.
    rewrite H.
...
(* Ans:
     The type [baz] has zero element because it's impossible to create
a value of type [baz]. The reason is that if we want to get a value of type [baz], we have to pass a value of type baz to [Baz1] or [Baz2]. However, [Baz1] has type baz->baz and [Baz2] has type baz->bool->baz. So, if we want to get a value of type baz we need to have a value of type baz first. It is impossible to get a value of type baz without having a value of type baz. In conclusion, the type [baz] has zero element. *)
*)
(** [] *)
