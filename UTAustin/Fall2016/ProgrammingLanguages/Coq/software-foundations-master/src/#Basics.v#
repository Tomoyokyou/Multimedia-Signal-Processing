...
Example test_plus1: (plus 0 3) = 3.
Proof. simpl. reflexivity.  Qed.

Example test_plus2: (plus 3 0) = 3.
Proof. simpl. reflexivity.  Qed.
...
Example test_minus2: (minus 9 9) = 0.
Proof. simpl. reflexivity.  Qed.

Example test_minus3: (minus 6 9) = 0.
Proof. simpl. reflexivity.  Qed.
...
Proof.
  ...
  {
    rewrite <- andb_oneIsFalse with c.
    rewrite -> H.
    destruct c.
    -reflexivity.
    -reflexivity.
...
Example test_bin_incr2:   bin_to_nat(incr (T BO)) = 1.
Proof. simpl. reflexivity.  Qed.

Example test_bin_incr3:   bin_to_nat(incr (M BO)) = 2.
Proof. simpl. reflexivity.  Qed.

Example test_bin_incr4:   bin_to_nat(incr (T(M BO))) = 3.
Proof. simpl. reflexivity.  Qed.

Example test_bin_incr5:   bin_to_nat(incr (T(incr(M BO)))) = 5.
Proof. simpl. reflexivity.  Qed.
(** [] *)
