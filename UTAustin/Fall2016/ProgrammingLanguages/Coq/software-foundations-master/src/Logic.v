...
(** Write an informal proof of [double_neg]:
   ...
   _Proof_:
(* Ans:
        First, we have a proposition [P]. 
        We want to show that it implies ~~P.
        By the definition of [~],
        we can know that ~~P means not (not P)
        [not P] means a proposition which is opposite to P.
        Then [not (not P)] means a proposition which is opposite to the opposition of P. Hence, it's easy to know that [not(not) P] equals to P.
  
        So, [P] implies [~~P], for any proposition [P].
...
Proof.
  ...
  apply H in H2. unfold not in H1. apply H1 in H2.
  apply H2.
Qed.
(** [] *)
...
(* Ans:
      We want to show that [P/\~P->false]
      P/\~P tells us that we have both propositions [P] and [~P].
      However, it's impossible to simultaneously satisfy these two at the same time.
      So, it's easy to know that [P/\~P->false]
...
Proof.
  ...
  split.
  - intros HP. apply H0 in HP. apply H3 in HP. apply HP.
  - intros HR. apply H4 in HR. apply H2 in HR. apply HR.
Qed.
(** [] *)
...
Proof.
  ...
  - intro. induction l.
    ...
    + inversion H. exists x.
      split.      
      * apply H0.
      * simpl. left. reflexivity.
      * apply IHl in H0. destruct H0.
        exists x0. destruct H0. split. apply H0.
...
- intros. inversion H. 
  ...
  + induction l.
    * simpl. apply H0.
    * simpl. right. apply IHl. right. apply H0. 
Qed.
...
Proof.
  ...
  }
  {
    ...
    -simpl. intros. inversion H. destruct H0.
      + rewrite <- H0. apply H1.
      + apply IHl with (x:=x0) in H2.
        * apply H2.
        * apply H0.
  }
Qed.
...
intros. 
unfold tr_rev. rewrite rev_append_eq_rev. apply app_nil_r.
Qed.
(** [] *)
...
Proof.
   intros b1 b2.
    split.
    - unfold andb. intros H1. 
      destruct b1 eqn:Desb1.
      + split. reflexivity. apply H1.
      + split. apply H1. inversion H1.
    - unfold andb. intros H1.
      destruct b1 eqn:Desb1.
      destruct H1.
      apply H0.
...
Proof.
  ...
  - unfold not. intros H.
    destruct (beq_nat x y) eqn:desbeqnat.
    + exfalso. apply H. apply beq_nat_true in desbeqnat. apply desbeqnat.
    + reflexivity.  
Qed.
...
Proof.
  ...
  split.
   ...
   {  generalize dependent l2. 
    ...
    - destruct l2 eqn:desL2.
      + simpl. intros H0. inversion H0.
      + intros. simpl in H0. 
        destruct (beq x a) eqn:DesBeq.
         * apply H in DesBeq. apply IHl1 in H0. 
            rewrite DesBeq. rewrite H0. reflexivity.
          * inversion H0.
}
  { intros.
    
    destruct l1 eqn:DesL1.
    - intros. simpl. rewrite <- H0. reflexivity.
    -  rewrite <- H0. simpl. rewrite beq_id. 
        + rewrite beq_list_id. 
          * reflexivity.
          * apply H.
        + apply H. 
...
{ induction l.
  ...
  - intros. simpl. simpl in H.
    split.
    + destruct (test x) eqn:DesTestX.
      * reflexivity.
      * destruct (forallb test l) eqn:desTest.
        inversion H.
        inversion H.
    + apply IHl. destruct (forallb test l) eqn:desTest.
      * reflexivity.
      * destruct (test x) eqn:DesTestX.
        inversion H.
        inversion H. 
...
(* Ans: 
    By theorem [forallb_true_iff], I've proved that if [forallb test l = true], then every element x of list l will all make [test x = true].

    However, I haven't proved that if [forallb test l = false] then there exists one of the element x in list l will make [test l = false].
    *)
(** [] *)
