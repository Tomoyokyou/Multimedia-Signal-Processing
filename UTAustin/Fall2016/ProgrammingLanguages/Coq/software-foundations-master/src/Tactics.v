...
Add LoadPath "/Users/AhPan/Documents/UTAustin/Fall2016/ProgrammingLanguages/Coq/software-foundations-master/src/".
Require Export Poly.
...
(* Ans:
        [apply] can use a known implication, such as a previous proved lemma,  a constructor, a hypothesis, to modify the proof state either backward or forward. (Backward means that if the goal matches the conclusion of the implication, in which case a subgoal is generated for each premise. And forward means that if some hypotheses satisfy the premise of the implication, in which case the hypothesis is replaced by the conclustion of the implication.)  
        On the other hand, [rewrite] is used to apply a known equality to either the goal or hypothesis, and then replace all occurences on one side by the other.
        When there is an implication without premises, both tactics both can be applied usefully. 
*)
*)
(** [] *)
...
Proof.
  ...
  rewrite -> H0. apply H. reflexivity. Qed.
...
Proof.
  ...
  - simpl. intros m H. destruct m as [|m'].
    ...
    + inversion H. rewrite <- plus_n_Sm in H1. 
      rewrite <- plus_n_Sm in H1. apply f_equal.
       rewrite -> plus_n_Sm in H. apply IHn'. inversion H1.
      reflexivity.
...
(* Ans:
      We want to prove [forall n, m : beq_nat n m = true -> n = m]
    
    First, we consider n as 0
    Then we consider m as 0
    So, it's easy to see that n = m = 0 & beq_nat 0 0 = true
    
    On the other hand, if we consider m as S m'
    beq_nat n m will become false. It doesn't satisfy the condition : beq_nat = true. So, when n = 0, m is impossible to be S m'.


    Secondly, we induct n as S O
    At first, we consider m as 0.
    beq_nat n m will become false. It doesn't satisfy the condition : beq_nat = true. So, when n = S n', m is impossible to be 0.
  
    Then  we consider m = S m' and suppose that when beq_nat n' m' = true -> n' = m'
     We want to show that S n' = S m'.
      By theorem [f_equal], we know that it's equivalent by showing n' = m'. From our supposition, we can know n' = m' is true. Hence S n' = S m'=> n = m

      By induction, we prove that [forall n,m : beq_nat n m = true -> n = m.]
...
Lemma nth_error_l_len_l : forall (X:Type) (l:list X),
...
Theorem app_length_cons_inverse : forall (X : Type) (l1 l2 : list X)
                                  (x : X) (n : nat),
     S (length (l1 ++ l2)) = n ->
     length (l1 ++ (x :: l2)) = n.
Proof.
      ...
      - intros n H. simpl. simpl in H. destruct n.
        + inversion H.
        + inversion H. apply IHl1 in H1. rewrite H1. rewrite H. reflexivity. 
...
Proof.
      ...
      - intros n0 H.
        ...
        + inversion H. apply IHl in H1.
          apply f_equal with (f:=S) in H1. 
          apply app_length_cons_inverse with (x:=x) in H1.
          rewrite H1. rewrite H. rewrite plus_n_Sm. reflexivity.   
Qed.
(** [] *)
...
Proof.
  ...
  destruct f eqn:f1.
  - destruct b in f1.
    + rewrite f1. rewrite f1. reflexivity.
    + destruct (f true) eqn:f2.
      *rewrite f2. reflexivity.
      * rewrite f1. reflexivity.
  - destruct b in f1.
    + destruct (f false) eqn:f3.
      * rewrite f1. reflexivity.
      * rewrite f3. reflexivity.
    + rewrite f1. rewrite f1. reflexivity.
Qed.
...
Lemma beq_nat_eq_tran : forall n m p,
beq_nat n m = true -> beq_nat n p = true -> m = p.
Proof.
    intros. induction n.
    - apply beq_nat_0_m in H. apply beq_nat_0_m in H0.
...
(* If l1 or l2 is [], it makes combine l1 l2 = [] by the definition of [combine]. And split [] will get ([], []). To ensure the [split] is the inverse of [combine], both l1 and l2 should be [] or both not be [].*)
...
Proof.
  ...
  - intros x1 lf H. simpl in H. 
    destruct (test x0) eqn:tx0.
    + inversion H. rewrite H1 in tx0. apply tx0.
...
Example test_existb1: existsb (beq_nat 5) [0;2;3;6] = false.
Proof. reflexivity. Qed.
Example test_existb2:  existsb (andb true) [true;true;false] = true.
Proof. reflexivity. Qed.
Example test_existb3:   existsb oddb [1;0;0;0;0;3] = true.
Proof. reflexivity. Qed.     
Example test_existb4:   existsb evenb [] = false.
Proof. reflexivity. Qed.      

Fixpoint neg_forallb {X:Type}(test:X->bool)(l:list X) : bool :=
  match l with
  | []     => false
  | h :: t => if test h then  true
                        else   neg_forallb test t
  end.


Definition existsb' {X:Type}(test:X->bool)(l:list X) : bool   :=   negb(forallb  (fun X => negb (test X)) l).

Example test_existb'1: existsb' (beq_nat 5) [0;2;3;6] = false.
Proof. reflexivity. Qed.
Example test_existb'2:  existsb' (andb true) [true;true;false] = true.
Proof. reflexivity. Qed.
Example test_existb'3:   existsb' oddb [1;0;0;0;0;3] = true.
Proof. reflexivity. Qed.     
Example test_existb'4:   existsb' evenb [] = false.
Proof. reflexivity. Qed.
